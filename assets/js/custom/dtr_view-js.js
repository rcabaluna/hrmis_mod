...
function formatDate(inputDate) {
    ...
    const month = dateParts[1] - 1; // Months are zero-based
...
$('#tbldtr').on('click', 'tbody > tr > td #btnleave', function () {
    var objsdata = $(this).data('json');
    $('#tblleave-details tr:nth-child(1) > td').text(formatDate(objsdata['dateFiled']));
    $('#tblleave-details tr:nth-child(2) > td').text(objsdata['leaveType']);

    if (objsdata['leaveFrom'] == objsdata['leaveTo']) {
        $('#tblleave-details tr:nth-child(3) > td').html(formatDate(objsdata['leaveFrom']));
    }else{
        $('#tblleave-details tr:nth-child(3) > td').html('<b>From</b> '+formatDate(objsdata['leaveFrom'])+' <b>To</b> '+formatDate(objsdata['leaveTo']));
    }
